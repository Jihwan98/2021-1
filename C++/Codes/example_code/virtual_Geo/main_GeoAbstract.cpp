...
int main() {
	// Geometric g("Red", 5, 5); // Can't be instantiated
	Geometric* gPtr; // Pointer 변수 생성은 가능하나, 객체 변수는 생성 불가. Abstract Class
	...
	A[4] = new Rect(50, 70, "purple", 9, 9);//new Geometric("white", 30, 30); 'Geometric': 추상 클래스를 인스턴스화할 수 없습니다.
	// 배열에 보관된 객체를 그린다.
	for (int i = 0; i < MAX; i++) {
		A[i]->draw();
	}
